...
export function PreviewPanel() {
	...
	useEffect(() => {
		const updatePreviewSize = () => {
			...
			if (isExpanded) {
				...
				availableHeight = container.height - paddingTop - paddingBottom - toolbarHeight - (toolbarHeight > 0 ? gap : 0);
			}
			...
			track.elements.forEach((element) => {
				...
				if (currentTime >= elementStart && currentTime < elementEnd) {
					...
					if (element.type === 'media') {
						mediaItem =
							element.mediaId === 'test' ? null : mediaFiles.find((item) => item.id === element.mediaId) || null;
					}
	...
	useEffect(() => {
		...
		const ensureAudioGraph = async () => {
			...
			audioGainRef.current!.gain.setValueAtTime(gainValue, audioContextRef.current!.currentTime);
		};
		...
		const scheduleNow = async () => {
			...
			// Collect all timeline media elements that can produce audio (audio files and videos)
			const audible: Array<{
				id: string; // media id
				elementStart: number;
				trimStart: number;
			...
			for (const track of tracksSnapshot) {
				for (const element of track.elements) {
					...
					if (!media) continue;
					// We now allow both explicit audio files and videos (which may contain audio streams)
	...
	useEffect(() => {
		const draw = async () => {
			...
			// Ensure size matches
			if (offscreenCanvasRef.current && (offscreenCanvasRef.current as HTMLCanvasElement).getContext) {
			...
			const offscreenCanvas = offscreenCanvasRef.current as HTMLCanvasElement | OffscreenCanvas;
			...
			await renderTimelineFrame({
				...
				mediaFiles,
				backgroundColor:
					activeProject?.backgroundType === 'blur' ? 'transparent' : activeProject?.backgroundColor || '#000000',
	...
	return (
		<>
			<div className="h-full w-full flex flex-col min-h-0 min-w-0 bg-panel rounded-sm relative">
				<div ref={containerRef} className="flex-1 flex flex-col items-center justify-center min-h-0 min-w-0">
...
function FullscreenToolbar({
	...
	return (
		<div data-toolbar className="flex items-center gap-2 p-1 pt-2 w-full text-foreground relative">
			<div className="flex items-center gap-1 text-[0.70rem] tabular-nums text-foreground/90">
				...
				<span>{formatTimeCode(totalDuration, 'HH:MM:SS:FF', activeProject?.fps || DEFAULT_FPS)}</span>
			</div>
			...
			<div className="flex-1 flex items-center gap-2">
				...
				>
					<div
						className={cn('absolute top-0 left-0 h-full rounded-full bg-foreground', !isDragging && 'duration-100')}
			...
			<div className="flex-1 flex items-center justify-center bg-background">
				<div
					...
					style={{
						...
						height: previewDimensions.height,
						background:
							activeProject?.backgroundType === 'blur' ? '#1a1a1a' : activeProject?.backgroundColor || '#1a1a1a',
					}}
				>
					{renderBlurBackground()}
					{activeElements.length === 0 ? (
...
function PreviewToolbar({
	...
	return (
		<div data-toolbar className="flex justify-end gap-2 h-auto pb-5 pr-5 pt-4 w-full">
			<div className="flex items-center gap-2">
				<Button variant="text" size="icon" onClick={toggle} disabled={!hasAnyElements} className="h-auto p-0">
					{isPlaying ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />}
				</Button>
				<Popover>
					<PopoverTrigger asChild>
						<Button variant="text" size="icon" className="h-auto p-0 mr-1" title="Toggle layout guide">
						...
						<div className="grid gap-4">
							<div className="space-y-2">
								...
								<p className="text-sm text-muted-foreground">
									Show platform-specific layout guides to help align your content with interface elements like profile
									pictures, usernames, and interaction buttons.
								</p>
							</div>
				...
				<Button variant="text" size="icon" className="size-4!" onClick={onToggleExpanded} title="Enter fullscreen">
					<Expand className="size-4!" />
				</Button>
			</div>
		</div>
